		...
		if (!fs.existsSync(TYPE_SDK_DIR)) {
			...
			if (!fs.existsSync(CHANNEL_SDK_DIR)) {
				...
				if (!fs.existsSync(path.join(TMP_FOLDER, 'Game'))) {
					...
					fsSync.copy(path.join(TMP_FOLDER, 'Game/assets'), Assets_path, {
						force: true
					});
				}
			}
...
function preparePkgfile(taskId) {
	...
	fsSync.copy(path.join(TMP_FOLDER, "CPSettings.txt"), path.join(TMP_FOLDER, "MainActivity/assets/CPSettings.txt"), {
		force: true
	});
	...
	fsSync.copy(path.join(TMP_FOLDER, 'Game/res/values/strings.xml'), path.join(TMP_FOLDER, 'MainActivity/res/values/app_strings.xml'), {
		force: true
	});
	...
	if (fs.existsSync(path.join(TMP_FOLDER, 'icon/drawable/app_icon.png'))) {
		fsSync.copy(path.join(TMP_FOLDER, 'icon'), path.join(TMP_FOLDER, 'MainActivity/res'), {
			force: true
		});
	}
}
...
function pkgFinsh(taskId) {
	...
	Travel(path.join(TMP_FOLDER, 'MainActivity/bin'), function(pathname) {
	...
	fsSync.copy(path.join(TMP_FOLDER, 'MainActivity/release.log'), path.join(SHARE_PATH, 'output/logs', BatchNo, taskId + '_release.log'), {
		force: true
	});
	fs.removeSync(path.join(TMP_FOLDER, 'MainActivity/release.log'));
...
function callShell(taskId, callback) {
	...
	if (preGamefile != 'err') {
		if (!fs.existsSync(path.join(TMP_FOLDER, 'Game/build.xml'))) {
			fs.removeSync(path.join(TMP_FOLDER, 'Game/project.properties'));
			AppendText(path.join(TMP_FOLDER, 'Game/project.properties'), "target=android-25");
			AppendText(path.join(TMP_FOLDER, 'Game/project.properties'), "android.library=true");

			child_process.execSync(
				ANDROID_TOOLS_BIN + ' update lib-project -p ' + path.join(TMP_FOLDER, 'Game'), {
					encoding: 'utf8',
					cwd: path.join(TMP_FOLDER, 'Game')
				},
				function(err, stdout, stderr) {
					if (err) {
						PkgLog(err);
						PkgLog('build.xml生成错误 : \n' + stderr);
					} else {
						PkgLog('build.xml生成结果 : \n' + stdout);
					}
				});
			PkgLog("创建build.xml");
			PkgLog(ANDROID_TOOLS_BIN + ' update' + ' lib-project' + ' -p ' + path.join(TMP_FOLDER, 'Game'));
		...
		var buildPath = path.join(TMP_FOLDER, 'MainActivity');

		PkgLog("开始进行编译，请稍等...");
		PkgLog(ANT_BIN + ' ' + CompileMode + ' -propertyfile' + ' version.properties' + ' -l ' + path.join(buildPath, 'release.log'));

		setTimeout(function() {
			child_process.exec(
				ANT_BIN + ' ' + CompileMode + ' -propertyfile' + ' version.properties' + ' -l ' + path.join(buildPath, 'release.log'), {
					encoding: 'utf8',
					cwd: buildPath
				},
				function(err, stdout, stderr) {
					if (err) {
						...
						PkgLog("\nOutput package log information.\n" + stdout);
						if (fs.existsSync(path.join(TMP_FOLDER, 'MainActivity/release.log'))) {
							PkgLog(" ----------------------- Release log ----------------------- : ");
							var rlog = fs.readFileSync(path.join(TMP_FOLDER, 'MainActivity/release.log'))
							if (os.platform() != 'win32') {
								PkgLog(rlog);
							} else {
								var buffer = new Buffer(rlog);
