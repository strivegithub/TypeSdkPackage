/**
 *@ TypeSDKClient
 *@ 2016-10
 *@ Copyright© 2016 www.typesdk.com. All rights reserved. 
 */

package com.type.sdk.application;

import android.app.Application;

import com.type.sdk.android.TypeSDKLogger;
import com.type.sdk.android.TypeSDKTool;
import com.type.sdk.android.youku.TypeSDKBonjour_youku;
//import com.type.sdk.notification.PushService;

public class TypeApplication extends Application{

	public TypeApplication() {
		
	}
	
	@Override
	public void onCreate() { 
		super.onCreate();
		TypeSDKLogger.e("TypeApplication");
		String buffStr = TypeSDKTool.getFromAssets(this, "CPSettings.txt");
        TypeSDKLogger.i(buffStr);
    	if(buffStr.length()>0)
    	{
    		TypeSDKBonjour_youku.Instance().platform.StringToData(buffStr);
    		TypeSDKLogger.i(TypeSDKBonjour_youku.Instance().platform.DataToString());
    	}
//		PushService.channelName = TypeSDKBonjour_youku.Instance().platform.GetData("channelName");
	}
}
