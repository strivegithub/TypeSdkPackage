...
public class TypeSDKBonjour_vivo extends TypeSDKBaseBonjour {

	private RequestQueue mQueue;
	public Context appContext;
	public Activity appActivity;
	//String KEY_SWITCH_ACCOUNT = "switchAccount";
	private VivoUnionManager  mVivoAccountManager;
	private boolean isLogin = false;

	private static class SingletonHandler {
		static final TypeSDKBonjour_vivo instance = new TypeSDKBonjour_vivo();
	}

	public static TypeSDKBonjour_vivo Instance(){
	...
	public void SendInfo(Context _in_context, String _in_data) {
		...
		try {
			...
			//记录用户角色信息			
			VivoUnionManager.vivoAccountreportRoleInfo(userData.GetData(AttName.ROLE_ID),
					userData.GetData(AttName.ROLE_LEVEL), 
					userData.GetInt(AttName.SERVER_ID) + "", 
					userData.GetData(AttName.ROLE_NAME),
					appContext, userData.GetData(AttName.SERVER_NAME));
	...
	public void ExitGame(Context _in_context) {
		if(exitGameListenser()){
			...
			if(mVivoAccountManager != null){
				mVivoAccountManager.hideVivoAssitView(appContext);
				//释放登录相关资源
				mVivoAccountManager.unRegistVivoAccountChangeListener(accountListener);
				//注销支付 监听  
				mVivoAccountManager.cancelVivoPaymentAndRecharge(payResultListener);
			}
			System.exit(0);			
		}
	...
	public void onResume(Context context) {
		...
		if(isLogin){
			if(mVivoAccountManager != null){
				TypeSDKLogger.e("showVivoAssitView");
				mVivoAccountManager.showVivoAssitView(context);
			} else {
				mVivoAccountManager = new VivoUnionManager(context);
				mVivoAccountManager.showVivoAssitView(context);
				TypeSDKLogger.e("mVivoAccountManager is null");
			}
		}
		...
		mainHandler.post(new Runnable() {
			...
			public void run() {
				try {
					...
					mVivoAccountManager = new VivoUnionManager(appContext);
					//账号登录退出监听
					mVivoAccountManager.registVivoAccountChangeListener (accountListener);
					//账号支付结果监听
					mVivoAccountManager.initVivoPaymentAndRecharge(appContext, payResultListener);
					mVivoAccountManager.bindUnionService();
					TypeSDKNotify_vivo notify = new TypeSDKNotify_vivo();
					notify.Init();
	...
	OnVivoAccountChangedListener accountListener = new OnVivoAccountChangedListener() {
		...
		public void onAccountLogin(String name, String openid, String authtoken) {
			...
			TypeSDKLogger.e("name="+name+", openid="+openid+", authtoken="+authtoken);
			isLogin = true;
			mVivoAccountManager.showVivoAssitView(appContext);
			TypeSDKNotify_vivo notify = new TypeSDKNotify_vivo();
		...
		public void onAccountLoginCancled() {
			TypeSDKLogger.e("onAccountLoginCancled");
			// TODO Auto-generated method stub
	...
	private void vivoLogin() {
		...
		mainHandler.post(new Runnable() {
			...
			public void run() {
				...
				//调用登录接口
				mVivoAccountManager.startLogin(platform.GetData(AttName.APP_ID));
...
//					
					...
					//调用支付接口进行支付
					mVivoAccountManager.payment(appContext, localBundle);
	...
	OnVivoPayResultListener  payResultListener = new OnVivoPayResultListener() {
		...
		public void payResult (String transNo, boolean pay_result,
			...
			PayResultData payResult = new PayResultData();
			TypeSDKNotify_vivo notify = new TypeSDKNotify_vivo();
			TypeSDKLogger.i("trabsNo："+transNo+"pay："+pay_result+"result_code:"+result_code+"pay_msg:"+pay_msg);
			if(result_code.equals("9000")){
				payResult.SetData(AttName.PAY_RESULT, "1");
				payResult.SetData(TypeSDKDefine.AttName.PAY_RESULT_REASON,
						"SUCCESS");
				notify.Pay(payResult.DataToString());
		...
		@Override
		public void rechargeResult(String arg0, boolean arg1, String arg2,
				String arg3) {
			// TODO Auto-generated method stub
