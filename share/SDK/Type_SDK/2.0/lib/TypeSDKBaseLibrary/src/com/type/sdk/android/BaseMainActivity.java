...
import com.type.sdk.tools.NetworkChangeReceiver;
...
public class BaseMainActivity extends UnityPlayerActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
    	...
    	FtpUploadUtil.fileFolder = platformBaseData.GetData(AttName.CP_ID);
    	final String switchConfigUrl = platformBaseData.GetData(AttName.SWITCHCONFIG_URL);
		...
		new Thread(){
			...
			public void run(){
				try {
					...
					String controllerMessage = HttpUtil.http_get(switchConfigUrl);
					//TypeSDKLogger.d("config=" + controllerMessage);
					//openPay = TypeSDKTool.openPay(sdkName, controllerMessage);
					
					TypeSDKTool.ctrlMessage(controllerMessage,_in_context, sdkName);
					getItemList(controllerMessage, cp_id, channel_id);
					if(FtpUploadUtil.uploadFiles((Activity)_in_context)){
						if(!TypeSDKTool.openLogCollector(controllerMessage
								,BaseMainActivity.sdkName)){
								TypeSDKLogger.i("close log collector");
								CrashHandler.flag = false;
								try {
									String cmds = "logcat -c ";
									Runtime.getRuntime().exec(cmds);				
									TypeSDKTool.clearFolder(Environment.getExternalStorageDirectory()
											.toString() + "/."+sdkName+"/CatchLogs");
								} catch (Exception e) {
